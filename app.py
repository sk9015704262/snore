...
# classes = ['Snoring', 'No-snoring', "Male-Snore", "Female-snore"]
# labelencoder = LabelEncoder()
# labelencoder.fit(classes)

# model_path = r'saved_models\audio_classification18_90(2).keras'
...
#         print(predicted_probabilities[0],"snore data", predicted_label)



#         print(f"Probability of Snore: {snore_prob:.4f}")
#         print(f"Probability of No-Snore: {no_snore_prob:.4f}")
...
# # def analyze_audio_directly(audio_binary):
# #     print(type(audio_binary), "analyze_audio_directly")
# #     try:
# #         audio_buffer = io.BytesIO(audio_binary)
# #         audio, sample_rate = librosa.load(audio_buffer, res_type='kaiser_fast')
        
# #         # Prepare audio features using the new model's approach
# #         features = prepare_audio_for_prediction(audio_buffer)
        
# #         if features is None:
# #             return {"error": "Error processing audio file"}
        
# #         # Make prediction
# #         predicted_probabilities = model.predict(features, verbose=0)
# #         predicted_label = np.argmax(predicted_probabilities, axis=1)
...
# #         if predicted_label[0] in range(len(classes)):
# #             prediction_class = classes[predicted_label[0]]
# #             print(prediction_class)
# #         else:
# #             return {"error": f"Unexpected prediction label: {predicted_label[0]}"}
        
# #         confidence = predicted_probabilities[0][predicted_label[0]]
...
# #         # Additional analysis if snoring is detected
# #         if prediction_class == 'Snoring':
# #             rmse = librosa.feature.rms(y=audio)
# #             rmse_db = librosa.amplitude_to_db(rmse, ref=np.max)
# #             average_intensity = np.mean(rmse_db)
            
# #             target_dB = 75 if platform.system() == "Linux" else 60
# #             intensity = float(average_intensity + target_dB)
